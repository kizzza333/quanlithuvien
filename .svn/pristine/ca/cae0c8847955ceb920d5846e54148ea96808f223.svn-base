...
class nxb_m extends connectDB{
    ...
    function delete_thanhphan($manxb){//thực hiện xóa các sách có mã nhà xuất bản nếu nhà xuất bản đó bị xóa
        $sql="DELETE FROM nhaxuatban WHERE MaNXB= N'$manxb'";
        return mysqli_query($this->con,$sql);
    }
    function update($manxb, $tennxb,$dienthoai, $email,$diachi){
        $sql="UPDATE nhaxuatban SET TenNXB='$tennxb' , DienThoai='$dienthoai', Email='$email',DiaChi='$diachi' WHERE MaNXB='$manxb'";
        return mysqli_query($this->con,$sql);
    }
}
?>