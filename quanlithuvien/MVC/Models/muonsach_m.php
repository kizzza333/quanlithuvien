...
class muonsach_m extends connectDB{
    public function muonsachphieu_ins($maphieu,$macanbo,$thedocgia,$ngaylap,$ngayhentra,$ngayketthuc,$trangthai){
        $sql = "INSERT INTO phieu values ('$maphieu','$macanbo','$thedocgia','$ngaylap','$ngayhentra','$ngayketthuc','$trangthai')";
    ...
    public function muonsach_checktrungmaDG($maphieu,$theDG){
        $sql = "select * from phieu where MaPhieu = '$maphieu' and TheDG = '$theDG'  ";
        $dl =  mysqli_query($this->con,$sql);
        $kq = false;
        if(mysqli_num_rows($dl)>0){
            $kq = true;
    ...
    public function muonsach_find($theDG){
